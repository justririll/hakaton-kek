...
<template>
  <div class="stack">
    ...
    <div class="card">
      ...
      <p class="muted sub">
        Центры объединены в группы по похожему составу аудитории. Такая группа называется кластером.
      </p>
      <ChartGuide>
        <li><strong>Одна точка — один центр.</strong> На всех мини-графиках показаны те же центры. Синим выделена группа из заголовка, серым — все остальные. Число рядом с названием — количество центров в группе.</li>
        <li><strong>Близкие точки обычно означают похожий состав аудитории.</strong> Далёкие — больше различий. Положение выше или правее не означает, что центр лучше работает.</li>
        <li><strong>Это карта сходства, а не географическая карта.</strong> Горизонталь и вертикаль объединяют несколько показателей состава аудитории, поэтому на них нет единиц вроде рублей или людей. Размер точки выделяет группу и не показывает размер аудитории.</li>
        <li>Карта показывает только два направления различий и упрощает данные. Для самого разбиения использовано {{ clusters.components }} направлений, сохраняющих {{ variance }} % различий исходных показателей.</li>
        <li>Наведите на точку, чтобы узнать название центра и аудиторию. «Силуэт» показывает, насколько центр подходит своей группе: ближе к 1 — подходит лучше, около 0 — на границе групп, ниже 0 — может быть ближе к другой группе.</li>
      </ChartGuide>
      <div class="panels">
